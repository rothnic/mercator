...
| 7 | I01-F5-T7 | Restore Fastify resolution in Vitest integration tests. | Update test/bundler config so `fastify` loads during service integration tests. | I01-F5-T2 | Todo | `pnpm test` currently fails because Vitest cannot resolve the `fastify` module while loading `createServer`. |
