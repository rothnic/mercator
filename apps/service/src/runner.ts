...
export async function runScraper(options: RunOptions): Promise<RunResult> {
  ...
  const knowledge = await loadDomainKnowledge(runtimePaths.domainKnowledgeDir, resourceId);
  ...
  await log(
    'generateScript',
    scriptDecision.reused ? 'Reused stored extractor script.' : 'Synthesized new Cheerio extractor script.'
  );

  const execution = runCheerioScript({ code: scriptDecision.script, html: htmlResult.html });
  if (execution.error) {
  ...
  if (!scriptDecision.reused && validation.ok) {
    ...
    await saveDomainKnowledge(runtimePaths.domainKnowledgeDir, updatedKnowledge);
    await log('persistScript', `Persisted script for ${resourcePath}.`);
  }

  const domainKnowledgePath = createDomainKnowledgePath(runtimePaths.domainKnowledgeDir, resourceId);
...
if (isMainModule()) {
  ...
  runScraper({ url })
    .then((result) => {
      ...
      console.info(`[run:${status}] title=${result.validation.value?.title ?? ''} price=${result.validation.value?.price ?? ''}`);
